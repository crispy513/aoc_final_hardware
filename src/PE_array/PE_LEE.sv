...
//
// Revisions:     2026/4/26  Finish module with ring ifmap spad, clock 
...
module PE_LEE (
    ...
    input [`CONFIG_SIZE-1:0] i_config,    // config signal, determine mode and
                                          // mapping parameter
    input [`DATA_BITS-1:0] ifmap,         // input ifmap pixels (32-bit here)
    input [`DATA_BITS-1:0] filter,        // input filter weights (32-bit here)
    input [`DATA_BITS-1:0] ipsum,         // input partial sum (32-bit here)
    ...
    output logic [`DATA_BITS-1:0] opsum,  // partial sum output
