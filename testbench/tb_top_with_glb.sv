...
module tb_top_with_glb;
    ...
    begin
        ...
        ifmap_base = '0;
        ifmap_len = ifmap_cmp_len_expected;
        filter_base = '0;
        filter_len = filter_cmp_len_expected;
        ipsum_base = '0;
        ipsum_len = ipsum_cmp_len_expected;

        ifmap_bitmap_base = '0;
        ifmap_bitmap_len = ifmap_bmap_len_expected[BMAP_ADDR_W:0];
        filter_bitmap_base = '0;
        filter_bitmap_len = filter_bmap_len_expected[BMAP_ADDR_W:0];
        ipsum_bitmap_base = '0;
        ipsum_bitmap_len = ipsum_bmap_len_expected[BMAP_ADDR_W:0];

        opsum_dense_len = golden_dense_len[OPSUM_AW:0];
        opsum_base = '0;
        opsum_len = golden_dense_len[OPSUM_AW:0];
        opsum_bitmap_base = '0;
        opsum_bitmap_len = opsum_bmap_len_expected[BMAP_ADDR_W:0];

        start_ifmap = 1'b1;
        start_filter = 1'b1;
        start_ipsum = 1'b1;
        start_ifmap_bitmap = 1'b1;
        start_filter_bitmap = 1'b1;
        start_ipsum_bitmap = 1'b1;
        start_encode_opsum = 1'b1;
        start_capture_opsum = 1'b1;
        start_capture_opsum_bitmap = 1'b1;
    ...
    task automatic wait_for_pipeline_done;
        int cycles;
        bit saw_opsum_done;
        bit saw_opsum_bitmap_done;
        bit saw_opsum_encode_done;
    begin
        cycles = 0;
        saw_opsum_done = 1'b0;
        saw_opsum_bitmap_done = 1'b0;
        saw_opsum_encode_done = 1'b0;
        while (!(saw_opsum_done && saw_opsum_bitmap_done && saw_opsum_encode_done)) begin
            @(posedge clk);
            saw_opsum_done |= opsum_done;
    ...
    initial begin
`ifdef FSDB
        $fsdbDumpfile("simulation/tb_top_with_glb.fsdb");
        $fsdbDumpvars(0, tb_top_with_glb);
`elsif WV_NORMAL
        $dumpfile("testbench/tb_top_with_glb.vcd");
        $dumpvars(0, tb_top_with_glb);
`endif
